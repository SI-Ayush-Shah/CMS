...
export const useImageUpload = (options = {}) => {
  ...
  const addImages = useCallback(async (newFiles) => {
    ...
    try {
      ...
      const filesToProcess = totalAfterAdd > maxImages 
      ...
      if (validation.validFiles.length > 0) {
        ...
        const previewResults = await generateMultiplePreviews(validation.validFiles)
   ...
   */
  const removeImageByIndex = useCallback((index) => {
   ...
   */
  const uploadSingleImage = useCallback(async (imageObject) => {
    return new Promise((resolve, reject) => {
      // Simulate upload progress
      let progress = 0
      const interval = setInterval(() => {
        progress += 20 // Fixed increment for testing
        ...
        if (progress >= 100) {
          ...
          let shouldSucceed
          if (testMode) {
            shouldSucceed = testShouldSucceed
          } else {
            // Simulate random success/failure for testing
            const randomValue = Math.random()
            shouldSucceed = randomValue > 0.1 // 90% success rate
          }
          ...
          if (shouldSucceed) {
            ...
            resolve(`mock-url-${imageObject.id}`)
          } else {
            reject(new Error('Mock upload failure'))
          }
        }
      }, 100)
    })
   ...
   */
  const retryUpload = useCallback(async (imageId) => {
