...
export const useLoadingState = (options = {}) => {
   ...
   */
  const startOperation = useCallback((operationId, config = {}) => {
    const {
    ...
    return {
      operationId,
      abortController,
      updateProgress: (newProgress, newMessage) => updateProgress(operationId, newProgress, newMessage),
   ...
   */
  const completeOperation = useCallback((operationId, result = null) => {
    // Clear timeout
   ...
   */
  const withLoadingState = useCallback((asyncFunction, config = {}) => {
    return async (...args) => {
      const operationId = config.operationId || `operation-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
  ...
  useEffect(() => {
    ...
    const hasLoading = operationList.some(op => op.state === LoadingState.LOADING)
    const hasError = operationList.some(op => op.state === LoadingState.ERROR || op.state === LoadingState.TIMEOUT)
  ...
  // Computed values
  const operationList = Object.values(operations)
  const activeOperations = operationList.filter(op => op.state === LoadingState.LOADING)
  const completedOperations = operationList.filter(op => 
    op.state === LoadingState.SUCCESS || 
    op.state === LoadingState.ERROR || 
    op.state === LoadingState.TIMEOUT ||
    op.state === LoadingState.CANCELLED
  )
  const errorOperations = operationList.filter(op => 
  ...
  const progress = activeOperations.length > 0 
    ? activeOperations.reduce((sum, op) => sum + op.progress, 0) / activeOperations.length
    : 0
  ...
  return {
    ...
    isOperationLoading: (operationId) => operations[operationId]?.state === LoadingState.LOADING,
    getOperationProgress: (operationId) => operations[operationId]?.progress || 0,
