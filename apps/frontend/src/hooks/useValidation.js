...
export const useValidation = ({ clearOnSuccess = true } = {}) => {
  ...
  const validateText = useCallback(
    (text, options = {}) => {
      ...
      if (
        text &&
        text.length >= maxLength * warningThreshold &&
  ...
  const validateImages = useCallback(
    (images, options = {}) => {
      const { required = false, maxImages = 10, minImages = 0 } = options;
  ...
  const validateForm = useCallback(
    async (formData, options = {}) => {
      ...
      try {
        ...
        const { textOptions = {}, imageOptions = {} } = options;
