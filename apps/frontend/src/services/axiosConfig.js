...
const calculateRetryDelay = (retryCount) => {
  return RETRY_CONFIG.retryDelay * Math.pow(RETRY_CONFIG.retryDelayMultiplier, retryCount)
}
...
export const classifyNetworkError = (error) => {
  ...
  if (status >= 400 && status < 500) {
    return {
      ...
      message: error.response.data?.message || 'Request failed. Please check your input and try again.',
      isRetryable: false
    }
  }
...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (import.meta.env.VITE_NODE_ENV === 'development') {
      const duration = config.metadata.startTime ? Date.now() - config.metadata.startTime : 'unknown'
    ...
    const shouldRetry = errorClassification.isRetryable && 
    ...
    if (shouldRetry) {
      ...
      if (import.meta.env.VITE_NODE_ENV === 'development') {
        console.log(`🔄 Retrying request (attempt ${config.metadata.retryCount}/${RETRY_CONFIG.maxRetries}) after ${delay}ms`)
      }
