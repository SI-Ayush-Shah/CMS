import { db } from '../db'
import { socialMediaPosts, type NewSocialMediaPost, type SocialMediaPost } from '../db/schema'
...
interface Dependencies { db: typeof db }

export function createSocialMediaPostRepository({ db }: Dependencies): SocialMediaPostRepository {
  ...
  return {
    ...
    async listByFeedItem(feedItemId: string): Promise<SocialMediaPost[]> {
      ...
      const rows = await db.select().from(socialMediaPosts).where(eq(socialMediaPosts.feedItemId, feedItemId))
      return rows
    }
  }
}
